...
Adobe Experience Platform Edge Networkでは、複数の製品向けのデータを1つのエンドポイントに送信し、各製品に適切な情報を転送できます。 これは、Streaming Media Collectionを実装する際にお勧めの方法であり、Adobe AnalyticsとCustomer Journey Analyticsの両方を1つのインストルメントでサポートする唯一の方法です。
...
1. **互換性のあるAdobe ソリューションを確認します。** 次の少なくとも1つの実装が動作している必要があります。
   ...
   * [Adobe Analytics](https://experienceleague.adobe.com/docs/analytics/implementation/home.html?lang=ja) – 同じデータストリームを介してCJAと共に、またはの代わりにサポートされます
   * [Adobe Journey Optimizer](https://experienceleague.adobe.com/docs/journey-optimizer.html?lang=ja)または[Real-Time Customer Data Platform](https://experienceleague.adobe.com/docs/real-time-customer-data-platform.html?lang=ja) – これらのいずれかを設定する場合は、**[!UICONTROL Adobe Experience Platform]** サービスをデータストリームに追加します
...
**インコード**&#x200B;実装では、SDK呼び出しをアプリケーションのソースコードに直接書き込みます。 **タグを使用**&#x200B;実装では、[Adobe Experience Platform タグ &#x200B;](https://experienceleague.adobe.com/en/docs/experience-platform/tags/home)を使用します。これにより、アプリケーションコードを変更することなくトラッキングルールを設定およびデプロイできます。 自社の導入ワークフローに最適なアプローチを選択してください。
