...
メディアバックエンドは、ミュート状態の開始イベントごとに`mediaReporting.states[]`の`mute` エントリの`count` フィールドを増分します。 この指標は、クローズ呼び出しで報告されます。
...
| Customer Journey Analytics | [`mediaReporting.states[]`](https://experienceleague.adobe.com/ja/docs/experience-platform/xdm/data-types/media-reporting-details) エントリ （`name = "mute"`、フィールド `count`） |
