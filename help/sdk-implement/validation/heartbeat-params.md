...
| l:asset:duration／l:asset:length <br/>（length duration から名前を変更） | VideoInfo | （必須）<br/><br/>メインアセットのビデオアセットの長さ。 |
...
| s:sp:channel | MediaHeartbeatConfig | （オプション）<br/><br/>ユーザーがコンテンツを視聴しているチャネル。モバイルアプリの場合、アプリ名。Web サイトの場合、ドメイン名。 |
