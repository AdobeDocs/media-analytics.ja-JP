...
# ハートビートパラメーターの説明{#heartbeat-parameter-descriptions}

アドビがハートビートサーバー上で収集および処理するハートビートパラメーターのリスト:
...
| `l:event:prev_ts` | R | メディア SDK | このセッションにおける同じタイプの最後のイベントのタイムスタンプ。The value is `-1` |
...
| `s:sc:rsid` | R | レポートスイート ID（複数の場合もあります） | レポート送信先の SiteCatalyst RSID。 |
| `s:sc:tracking_server` | R | `MediaHeartbeatConfig` | SiteCatalyst トラッキングサーバー。 |
