---
title: ライブメインコンテンツ
description: メディアSDKを使用したライブコンテンツの追跡方法の例を示します。
...
ライブストリームの場合、レポートで、アナリストが、ユーザーが参加し、24時間表示でLIVEストリームを離脱する時点を特定できるように、再生ヘッド値をその日の午前0時(UTC)からの秒数として設定する必要があります。

### 開始時

ライブメディアの場合、ユーザーがストリームの再生を開始する際に、その日の午前0時(UTC)からの秒数に`l:event:playhead`を設定する必要があります。 これは、VOD とは対照的です（VOD では再生ヘッドを「0」に設定します）。

例えば、ライブストリーミングイベントが午前 0 時に開始し、24 時間実行するとします（`a.media.length=86400`; `l:asset:length=86400`）。次に、ユーザーが午後 12：00 にそのライブストリームの再生を開始するとします。このシナリオでは、`l:event:playhead`を43200(UTCの午前0時から12時間（秒）)に設定する必要があります。

### 一時停止時

ユーザーが再生を一時停止する場合、再生の開始時に適用されたのと同じ「ライブ再生ヘッド」ロジックが適用される必要があります。ユーザーがライブストリームの再生に戻ったら、午前0時(UTC)からの新しい秒数（_ではない_）に従って、ユーザーがライブストリームを一時停止した時点に`l:event:playhead`を設定する必要があります。
