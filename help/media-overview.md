---
title: Adobe streaming mediaの概要
description: Adobeのストリーミングメディアソリューションを使用して、コンテンツ、オーディオ、広告に関する強力なinsightを取得します。
...
# Adobe ストリーミングメディアサービスの概要

![バナー](./assets/media_analytics_banner.png)

Adobeのストリーミングメディアサービスは、ストリーミングメディアプロバイダー向けに、オーディオ、動画、広告などのストリーミングメディアコンテンツの強力な収集、測定、パーソナライゼーションツールを提供します。 ストリーミングメディア指標を、Audience Analytics、モバイル、クロスデバイス分析などの機能と組み合わせることができます。
...
>[!IMPORTANT]
>
>ストリーミングメディアサービスを導入するには、Adobeの営業担当者またはAdobeのアカウントチームに連絡して、Customer Journey Analytics Streaming Media Collection アドオンまたはAdobe Analytics for Streaming Media アドオンが製品ポートフォリオに含まれていることを確認してください。

## 主な特長

ストリーミングメディアサービスの利点には、リアルタイムのモニタリング、詳細な分析、実用的なインサイト、収益化の機会などが含まれます。

* **リアルタイム分析**：複数のチャネルにわたり、メディア開始などの主要パフォーマンス指標を利用して、アクションにつながる意思決定をリアルタイムで行います。

  デュレーション、停止、開始の詳細をほぼリアルタイムで取得し、ストリーミングメディアサービスで動画とオーディオの指標を評価して組み合わせることができます。 これらのインサイトにより、顧客の視聴習慣を把握し、高度にパーソナライズされたレコメンデーションでエンゲージメントを強化できます。

* **エンゲージメントの促進**：バッファリングイベントを減らしコンテンツ内で広告を再生する場所とタイミングを把握することによりユーザーエンゲージメントを強化して、繰り返し訪問を実現する、押し付けがましくないスムーズなエクスペリエンスを提供します。

* **全体像**：すべてのコンテンツディストリビューターで複数のデータポイントを組み合わせて、すべてのメディアアクティビティの全体像を把握します。 可能なあらゆるチャネルをまたいでエンゲージメントとビュー/リスニングを測定。

  ストリーミングメディアサービスを活用すれば、サイトとストリーミングアプリをまたいでカスタマージャーニー全体を追跡し、顧客のパスと関心を可視化して、より強化されたレコメンデーションを提供したり、顧客体験をパーソナライズできます。  メディア測定を使用すれば、データを複数のディメンションやセグメントに分類し、完全で詳細な分析をおこなうのに必要なメタデータをすべてキャプチャできます。 その後、データを分析し、成功基準を、完全に消費されたメディア、平均滞在時間、完了した広告に関連付けることができます。
...
ストリーミングメディアサービスのトラッキングデータは、Media for Edge Network SDK/Extension、Media Extension with Tags、Media SDK、Media Edge API、またはMedia Collection APIを使用してプレーヤーから収集されます。
