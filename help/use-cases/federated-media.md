...
* **ストリーミングメディアコレクションの実装：** 送信者は、フェデレーティッドデータセットの一部となるすべてのプレーヤーにストリーミングメディアコレクションを実装する必要があります。 ストリーミングメディアデータのみがフェデレーションに使用できます。 詳しくは、[Adobe Streaming Media Collection の概要 ](/help/media-overview.md) を参照してください。
