...
これは、スキップボタンのクリックなどでユーザーが広告をスキップできるようにアプリケーションが準備されていることを除いて、[プリロール広告のある VOD 再生](/help/use-cases/tracking-scenarios/vod-preroll-ads.md)と同じシナリオです。
...
| 広告ブレークが終了します。 | `trackEvent:AdBreakComplete` | | 広告がスキップされた場合でも必要です。 このイベントを使用しない場合、広告イベントは無視され、スキップされた広告期間はメインコンテンツとしてカウントされます。 |
...
パラメーターは、広告完了呼び出しがない場合を除き、プレロール広告](/help/use-cases/tracking-scenarios/vod-preroll-ads.md) シナリオでの[VOD再生のパラメーターと同じです。 スキップして広告枠を閉じるには、引き続き`AdBreakComplete`が必要です。
