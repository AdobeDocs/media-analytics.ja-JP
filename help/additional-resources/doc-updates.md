...
* この更新は、主に、JavaScriptおよびOTTプラットフォームの2.2メディアSDKリリースに対応しています。
* JavaScriptおよびOTTプラットフォームの2.2メディアSDKリリースでは、iOSプラットフォームおよびAndroidプラットフォーム（2018年11月1日更新）で後述されているように、同じサポートが提供されています。
...
* 新しいドキュメントシステムへの移行に備えて、ファイルの名前が全般的に変更されました。すべてのDITAプリフィックス（c_， r_， t_）は、すべて削除されました。すべてのアンダースコア（「_」）がハイフン（「-」）に置き換えられました。また、ファイル名がトピックのタイトルに近いものに変更されました。
